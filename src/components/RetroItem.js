...
const RetroItem = props => {
    ...
    const handleMoveItem = (item) => {
        ...
        moveItem({ variables: { input: { _id: props.id, itemId: props.item.itemId, description: props.item.description, likes: props.item.likes, type: props.item.type, action: 'move' } } })
    ...
    const [moveItem, { dataItem, loadingItem, errorItem }] = useMutation(MOVE_ITEM, {
        refetchQueries: [
            GET_ITEMS_BY_ITERATION, // DocumentNode object parsed with gql
            'allByIterationAndTeam' // Query name
        ],
    ...
    return (
        <div className="task">
            ...
            <div className="like-content" style={{position: "relative"}}>
                ...
                {props.item.type !== "actionItems" && props.item.type !== "kudos" && (                    
                    <div style={{position: "absolute", marginLeft: "14%", top: "2px", right: "8%"}}>
                        <button title='Move to Action Items' onClick={() => handleMoveItem(props)} className="btn-secondary like-review" style={{color: "white"}}><i className="material-icons right" style={{fontSize: "22px"}}>swap_horiz</i></button>
                    </div>
                )}
