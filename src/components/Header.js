...
const Header = props => {
    ...
    if(!loading && data) {
        console.log("last_iteration", data?.allRetrosByTeam[data?.allRetrosByTeam.length-1].iteration);
    ...
    return (
        <React.Fragment>
            <nav>
                ...
                <ul id="nav-mobile" className="right font-header hide-on-med-and-down">
                    ...
                    <li><Link to={"/board/" + props.team + "/" + data?.allRetrosByTeam[data?.allRetrosByTeam.length-1].iteration}>Board</Link></li>
                    )}
                ...
                <ul className="right font-header hide-on-med-and-up show-on-medium-and-down">
                    <li className="show-on-medium-and-down"><a className="dropdown-trigger show-on-medium-and-down" href="#!" data-target="dropdown1"><i className="material-icons right">menu</i></a></li>
                </ul>
                <ul id="dropdown1" className="dropdown-content show-on-medium-and-down">
                    {/* <li style={{position: "relative"}}><RetrospectiveModal teams={props.team} last_iteration={data?.allRetrosByTeam[data?.allRetrosByTeam.length-1].iteration} /> </li> */}
                    {/* <li style={{position: "relative"}}><Link to="#"><span style={{fontSize: "28px", position: "absolute", left: "0"}}>+</span> New Team</Link></li> */}
                    <li><a href="/retros">My Retros</a></li>
                    {data?.allRetrosByTeam.length > 0 && (
                    <li><a href={"/board/" + props.team + "/" + data?.allRetrosByTeam[data?.allRetrosByTeam.length-1].iteration}>Board</a></li>
                    )}
                    {props.team.toString()!=='' && (
                        <li style={{marginTop: "5px", paddingLeft: "16px", fontSize: "16px"}}><strong>{props.team.toString().toUpperCase()} TEAM</strong></li>
                    )}
                    {/* <li><Link to="#">My Teams &nbsp;</Link></li> */}
                </ul>
                </div>
